...
export function getControls() {
  if (GRAPH_IMPL === "cytoscape") {
    return [{name: "Zoom in", icon: Add, callback: cytoZoomIn}, {name: "Center", icon: CenterFocusStrong, callback: cytoFitTo}, {name: "Zoom Out", icon: Remove, callback: cytoZoomOut }]
  }
  else if (GRAPH_IMPL === "sigma") {
    return [{name: "Zoom in", icon: Add, callback: sigmaZoomIn}, {name: "Center", icon: CenterFocusStrong, callback: sigmaFitTo}, {name: "Zoom Out", icon: Remove, callback: sigmaZoomOut }]
  }
  else {
    return [{name: "Zoom in", icon: Add, callback: visZoomIn}, {name: "Center", icon: CenterFocusStrong, callback: visFitTo}, {name: "Zoom Out", icon: Remove, callback: visZoomOut }]
  }
}
